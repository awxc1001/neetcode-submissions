class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        # 同一时间再看冷却队列：有没有任务在这一秒“刑满释放”了？如果有，赶紧放回大池子（max_heap）。下一秒就可以process了
        ...
        while max_heap or cd_q:
            ...
            if max_heap:
                ...
                remain += 1
                # if remain still exist, add to cd_que and mark the correct time to return to queue for next second process
                if remain != 0:
                    return_time = cur_time + cd_time
                    info_tuple = (remain, return_time, t_name)
                    cd_q.append(info_tuple)
            
            #check if any in the q can be readded to maxheap pool
            if cd_q:
                #check earliest saved task can return or not
                return_time = cd_q[0][1]
                #do pop and heappush if match curr_time
                if return_time == cur_time:
                    remain, return_time, t_name = cd_q.popleft()
                    #extract needed info
