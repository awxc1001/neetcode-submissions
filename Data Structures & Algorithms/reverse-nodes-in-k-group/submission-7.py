...
class Solution:
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        #since reverse by k groups, after group reverse, remainder never changed
        #just return dummy.next which is the new head of entire reverse
        return dummy.next

    def reverse_k_nodes(self, node: ListNode, k: int):
        # 先往后往后走 k 步，定位到这组reverse后的next pointer
        reverse_group_next = node
        #this node will be the prev for our reverse
        for _ in range(k):
            reverse_group_next = reverse_group_next.next
        
    # 1 -> 2 -> 3 ->4 -> 5 k =3
    # 4 is reverse_group_next is prev
    # 1 is the tail after reverse, but also pointer to begin the reverse
    # 4 is prev, 1 is curr
    # after all reverse steps
    # 5<- 4 <- 1 <- 2 <- 3
    #.   1 still has a pointer, so can return new tail  3 is prev, curr is gone and we dont care
