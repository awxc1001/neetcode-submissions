...
class Solution:
    def cloneGraph(self, node: Optional['Node']) -> Optional['Node']:
        ...
        #use another visited to avoid scope error
        visited1 = set()
        def dfs_pointneibo(node):
            ...
            #locate the current
            curr_copy = node_copies[node]
            for neibo in node.neighbors:
                #locate the copy and the current
                neibo_copy = node_copies[neibo]
                # append into curr_copy neibo list
                curr_copy.neighbors.append(neibo_copy)
                #traverse all neibo
